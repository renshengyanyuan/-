 ...
 *
 * 好想你官方会员  小程序
 * 签到，必须到附近实体店去兑换吃的，自己决定跑不跑
 * cron 0 0,7 * * *  yml2213_javascript_master/hxn.js
 *
 * 7-14		签到，积分查询
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export hxn_data=' az @ az '   ,多账号用 换行 或 @ 分割
 * 抓vipnew.hxninfo.com下请求头的authorization填入变量
 * ck测试了三天没掉
 */
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.hxn_data : $.getdata('hxn_data')) || '';
 let msg, ck;
 let ck_status = true;
 let host = 'vipnew.hxninfo.com';
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "hxn_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/sign.json`,
			 headers: {
				"Host": "vipnew.hxninfo.com",
				"Content-Length": "14",
				"authorization": ck[0],
  ...
  */
   async function jifen() {
	try {
		...
		if (result?.status == 200) {
			DoubleLog(`查询成功，当前积分:${result.data.pointCurrent}`);
			await wait(5);
		} else {
